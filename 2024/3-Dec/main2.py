...
#-----------------------------------------
    print("TEST INPUT:")
    test_input = split_file_to_string_arr(remove_invalid_chars(get_data("./test-input.txt")))
    print(test_input)

    def check_str(str):
        string_to_search = str
        pattern = re.compile(r"mul\(\d{1,3},\d{1,3}\)")
        match string_to_search:
            case patturn:
                #print("match found", string_to_search)
                return True
        return False
    ...
    print(check_arr(test_input))

    print("\nCHECK PUZZLE FILE:")
    print(check_arr(puzzle_file))
